...
pub fn run_decode_bench(
    ...
    for _ in 0..warmup {
        ...
        while remaining > 0 {
            if model.decode_one(&mut s, gpu).is_none() {
                break;
            }
            remaining -= 1;
        }
...
pub fn run_cache_format_bench(
    ...
    formats
        ...
        .map(|(fmt, name)| {
            ...
            let ms_per_token = if decode_tokens > 0 {
                decode_ms / decode_tokens as f64
            } else {
                0.0
            };
            let tps = if ms_per_token > 0.0 {
                1000.0 / ms_per_token
            } else {
                0.0
            };
...
pub fn print_results(results: &[BenchResult]) {
    println!(
        "{:<22} {:>8} {:>8} {:>6} {:>8} {:>10} {:>12} {:>10}",
        "Mode", "Prompt", "Decode", "Conc", "Cache", "Prefill ms", "ms/token", "tok/s"
    );
    println!("{}", "-".repeat(92));
    for r in results {
        let prompt = if r.prompt_tokens > 0 {
            format!("{}", r.prompt_tokens)
        } else {
            "-".to_string()
        };
        let decode = if r.decode_tokens > 0 {
            format!("{}", r.decode_tokens)
        } else {
            "-".to_string()
        };
        let prefill = if r.prefill_ms > 0.0 {
            format!("{:.1}", r.prefill_ms)
        } else {
            "-".to_string()
        };
        let mpt = if r.decode_ms_per_token > 0.0 {
            format!("{:.2}", r.decode_ms_per_token)
        } else {
            "-".to_string()
        };
        let tps = if r.tokens_per_sec > 0.0 {
        ...
        println!(
            "{:<22} {:>8} {:>8} {:>6} {:>8} {:>10} {:>12} {:>10}",
            r.mode, prompt, decode, r.n_concurrent, r.cache_format, prefill, mpt, tps
        );
    }
}
